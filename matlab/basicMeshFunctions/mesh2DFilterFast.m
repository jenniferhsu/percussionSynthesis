function y = mesh2DFilterFast(ex, Nj, N, fs, inX, inY, outX, outY, B, A, plotOn)
%Y = mesh2DFilterFast(ex, Nj, N, fs, inX, inY, outX, outY, B, A, plotOn)
...
% allows you to use first order filters.
%
% inputs:
...
for n=1:N
    ...
    [upper(1:Nj,1), last_in(1:Nj,1)] = simple_filter(B, A, lowerOld(1:Nj,1), last_in(1:Nj,1)); 
    % right-most edge of mesh 
    [lower(1:Nj,Ns), last_in(1:Nj,2)] = simple_filter(B, A, upperOld(1:Nj,Ns), last_in(1:Nj,2)); 
    % highest edge of mesh
    [right(1,1:Nj), last_in(1:Nj,3)] = simple_filter(B, A, leftOld(1,1:Nj), last_in(1:Nj,3)');
    % lowest edge of mesh
    [left(Ns,1:Nj), last_in(1:Nj,4)] = simple_filter(B, A, rightOld(Ns,1:Nj)', last_in(1:Nj,4));
